...
bool CLZObject::Encrypt(uint16_t uiKeySet)
{
	...
	m_pHeader->dwEncryptSize = tea_encrypt((DWORD*)pbBuffer, (const DWORD*)pbBuffer, (const unsigned long)uiKeySet, m_pHeader->dwCompressedSize + 19);
	return true;
}
...
bool CLZObject::__Decrypt(uint16_t uiKeySet, BYTE* pbData)
{
	...
	tea_decrypt((DWORD*)pbData, (const DWORD*)(m_pbIn - sizeof(DWORD)), (const unsigned long)uiKeySet, m_pHeader->dwEncryptSize);
	return true;
}
