...
PyObject* netSendShopBuyPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendShopBuyPacket(bPos);
	return Py_BuildNone();
}
...
PyObject* netSendNotice(PyObject* poSelf, PyObject* poArgs)
{
	char* szName;
	if (!PyTuple_GetString(poArgs, 0, &szName))
		return Py_BuildException();
	...
	badge_notification_manager.ReceiveWhisper(szName);
#endif
	return Py_BuildNone();
}

auto netSeenNotice(PyObject* poSelf, PyObject* poArgs) -> PyObject*
{
	char* target_name;
...
PyObject* netSendOfflineShopBuyPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 1, &pos))
		return Py_BuildException();
	int itemID;
	if (!PyTuple_GetInteger(poArgs, 2, &itemID))
		return Py_BuildException();
	if (pos < 0)
		return Py_BuildException();
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendOfflineShopBuyPacket((DWORD)vid, (BYTE)pos, (DWORD)itemID);
	return Py_BuildNone();
}
...
PyObject* netSendRemoveOfflineShopItem(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 0, &bPos))
		return Py_BuildException();
	int dwItemID;
	if (!PyTuple_GetInteger(poArgs, 1, &dwItemID))
		return Py_BuildException();
	BYTE bTakeAll;
	if (!PyTuple_GetInteger(poArgs, 2, &bTakeAll))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::instance();
	rkNetStream.SendRemoveOfflineShopItem(bPos, (DWORD)dwItemID, bTakeAll);
	return Py_BuildNone();
}

PyObject* netSendOfflineShopChangePrice(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bPos;
	if (!PyTuple_GetInteger(poArgs, 0, &bPos))
		return Py_BuildException();
	...
	if (!PyTuple_GetInteger(poArgs, 1, &dwItemID))
		return Py_BuildException();
	long long llItemPrice;
	if (!PyTuple_GetLongLong(poArgs, 2, &llItemPrice))
		return Py_BuildException();
	BYTE bAllSameItem;
	if (!PyTuple_GetInteger(poArgs, 3, &bAllSameItem))
		return Py_BuildException();
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::instance();
	rkNetStream.SendOfflineShopChangePrice(bPos, dwItemID, llItemPrice, bAllSameItem);
	return Py_BuildNone();
}
...
PyObject* netSendTeleportOfflineShop(PyObject* poSelf, PyObject* poArgs)
{
	char* szShopOwnerName;
	if (!PyTuple_GetString(poArgs, 0, &szShopOwnerName))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::instance();
	rkNetStream.SendTeleportOfflineShop(szShopOwnerName);
	return Py_BuildNone();
}

PyObject* netSendTakeOutAllItemsOfflineShop(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::instance();
	rkNetStream.SendTakeOutAllItemsOfflineShop();
	return Py_BuildNone();
}

PyObject* netSendMoveItemOfflineShop(PyObject* poSelf, PyObject* poArgs)
{
	int iSlotPos;
	...
	if (!PyTuple_GetInteger(poArgs, 1, &iTargetPos))
		return Py_BuildException();
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::instance();
	rkNetStream.SendMoveItemOfflineMoney((WORD)iSlotPos, (WORD)iTargetPos);
	return Py_BuildNone();
}
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SendShopSellPacketNew",				netSendShopSellPacket,				METH_VARARGS },
		...
		{ "SendOfflineShopChangePrice", netSendOfflineShopChangePrice, METH_VARARGS },
		{ "SendDestroyOfflineShop", netSendDestroyOfflineShop, METH_VARARGS },
		{ "SendTeleportOfflineShop", netSendTeleportOfflineShop, METH_VARARGS },
		{ "SendTakeOutAllItemsOfflineShop", netSendTakeOutAllItemsOfflineShop, METH_VARARGS },
		{ "SendMoveItemOfflineShop", netSendMoveItemOfflineShop, METH_VARARGS },
...
#endif
		{ "SendNotice",								netSendNotice,								METH_VARARGS },
		{ "SeenNotice",								netSeenNotice,								METH_VARARGS },
