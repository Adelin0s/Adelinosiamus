...
#ifdef __VERSION__
DWORD CAccountConnector::GetExeVersion()
{
...
bool CAccountConnector::__AuthState_Process()
{
	...
	if (!__AnalyzePacket(HEADER_GC_MAINTENANCE, sizeof(TPacketGCMaintenance), &CAccountConnector::__AuthState_RecvMaintenance))
		return true;
#endif
...
#ifdef __MAINTENANCE__
bool CAccountConnector::__AuthState_RecvMaintenance()
{
	TPacketGCMaintenance p;
	if (!Recv(sizeof(TPacketGCMaintenance), &p))
		return false;
	if (m_poHandler)
		PyCallClassMemberFunc(m_poHandler, "SetMaintenance", Py_BuildValue("(iiiis)", p.maintenance.mode, p.maintenance.iStartTime, p.maintenance.iEndTime, p.maintenance.status, p.maintenance.reason));
	return true;
}
#endif
...
#ifdef __VERSION__
		LoginPacket.version = GetExeVersion();
#endif
		ClearLoginInfo();
