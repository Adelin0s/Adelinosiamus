...
void CInstanceBase::RefreshModelScale()
{
	if (IsNewPet())
	{
		...
		m_GraphicThingInstance.SetScalePosition(fScale, fScale, fScale);
	}
}
#endif
...
bool CInstanceBase::Create(const SCreateData& c_rkCreateData)
{
...
#endif
	const auto npcRaceIndex = CPythonNonPlayer::Instance().GetNPCWear(c_rkCreateData.m_dwRace, CPythonNonPlayer::NPC_WEAR_RACE);
	...
	if (npcRaceIndex != -1)
	{
		...
		const auto mount = nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, CPythonNonPlayer::NPC_WEAR_MOUNT);
		if (mount != 0)
			MountHorse(mount);
		SetArmor(nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, CPythonNonPlayer::NPC_WEAR_ARMOR));
		SetWeapon(nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, CPythonNonPlayer::NPC_WEAR_WEAPON));
		SetHair(nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, CPythonNonPlayer::NPC_WEAR_HAIR));
		SetAcce(nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, CPythonNonPlayer::NPC_WEAR_ACCE));
		//for (int i = 0; i < CPythonNonPlayer::CHR_SHINING_NUM; i++)
		//	SetShining(i, nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, CPythonNonPlayer::NPC_WEAR_SHINING_WEAPON + i));
	}
	else
	{
	...
	if (IsShop())
	{
		if (m_myShop == 0 && c_rkCreateData.m_stName == std::string(CPythonPlayer::Instance().GetName()))
		{
			m_myShop = __AttachEffect(EFFECT_REFINED + EFFECT_MYOFFSHOP);
			//m_GraphicThingInstance.SetScaleWorld(2, 2, 2);
		}
	}
#endif
	...
	if (dwBodyColor)
	{
		SetColorOperation(c_rkCreateData.m_dwRace, dwBodyColor);
	}
#else
	if (!IsPC())
	{
...
void CInstanceBase::__Create_SetName(const SCreateData& c_rkCreateData)
{
	...
	if (IsEnemy() && CPythonSystem::Instance().IsShowMobAIFlag() && CPythonNonPlayer::Instance().IsAggressive(GetRace()))
	{
		...
		SetNameString(m_dwRace == 3970 ? " " : strName.c_str(), strName.length());
	}
	else
	{
		SetNameString(m_dwRace == 3970 ? " " : c_rkCreateData.m_stName.c_str(), c_rkCreateData.m_stName.length());
	}
#else
...
void CInstanceBase::Update()
{
	...
	AttackProcess();
	MovementProcess();
	RemoveExpiredEffects(std::chrono::milliseconds(5000));
	...
	auto* pythonCharacterManager = CPythonCharacterManager::InstancePtr();

	if (pythonCharacterManager != nullptr)
	{
		auto* pMainInstance = pythonCharacterManager->GetMainInstancePtr();
		if (pMainInstance != nullptr)
		{
			if (IsPC())
			{
				if (GetDistance(pMainInstance) >= CPythonSystem::instance().GetEFFECT())
				{
			...
			else if (IsNPC() && !IsShop() && !IsHorse() && !IsNewPet() && !IsPet() && !IsBuffi() && !IsPet())
			{
				if (GetDistance(pMainInstance) <= CPythonSystem::instance().GetNPC())
				{
					if (IsFlag() ? false : GetDistance(pMainInstance) >= CPythonSystem::instance().GetEFFECT())
					{
						m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
					}
					else
			...
			{
				if (GetDistance(pMainInstance) <= CPythonSystem::instance().GetMOUNT())
				{
					if (GetDistance(pMainInstance) >= CPythonSystem::instance().GetEFFECT())
					{
						m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
			...
			{
				if (GetDistance(pMainInstance) <= CPythonSystem::instance().GetMOB())
				{
					if (GetDistance(pMainInstance) >= CPythonSystem::instance().GetEFFECT())
					{
						m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
			...
			{
				if (GetDistance(pMainInstance) <= CPythonSystem::instance().GetPET())
				{
					if (GetDistance(pMainInstance) >= CPythonSystem::instance().GetEFFECT())
					{
						m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
			...
			{
				if (GetDistance(pMainInstance) <= CPythonSystem::instance().GetSTONE())
				{
					if (GetDistance(pMainInstance) >= CPythonSystem::instance().GetEFFECT())
					{
						m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
			...
			else if (GetRace() >= 30000 && GetRace() <= 30007)
			{
				TPixelPosition shopPos;
				NEW_GetPixelPosition(&shopPos);
				if (shopPos.x < 1 && shopPos.y < 1)
				{
					m_GraphicThingInstance.SetActiveAllAttachingEffect();
					return;
				}

				if (GetDistance(pMainInstance) <= CPythonSystem::instance().GetSHOP())
				{
					if (GetDistance(pMainInstance) >= CPythonSystem::instance().GetEFFECT())
					{
						m_GraphicThingInstance.SetDeactiveAllAttachingEffect();
	...
	if (GetRace() >= 30000 && GetRace() <= 30007)
	{
		if (pythonCharacterManager != nullptr)
		{
			...
			if (pMainInstance != nullptr)
			{
				if (GetDistance(pMainInstance) <= CPythonSystem::instance().GetSHOP())
				{
					m_GraphicThingInstance.ReleaseAlwaysHidden();
				}
				else
				{
					m_GraphicThingInstance.ApplyAlwaysHidden();
				}
			}
		}
	}

	if (IsNPC() && !IsShop() && !IsHorse() && !IsNewPet() && !IsPet() && !IsBuffi()) // SPER CA NU SE FUTE
	{
		if (pythonCharacterManager != nullptr)
		{
			auto* pMainInstance = pythonCharacterManager->GetMainInstancePtr();
			if (pMainInstance != nullptr)
			{
				if (GetDistance(pMainInstance) <= 10000.0f)
				{
					if (GetDistance(pMainInstance) <= CPythonSystem::instance().GetNPC())
					{
						m_GraphicThingInstance.ReleaseAlwaysHidden();
	...
	if (Frustum::Instance().currentMapNameSave == "metin2_map_lobby")
	{
		if (IsPC())
		{
			if (pythonCharacterManager != nullptr)
			{
				...
				if (pMainInstance != nullptr)
				{
					if (Frustum::Instance().alwaysHiddenLobby == true)
					{
						m_GraphicThingInstance.ReleaseAlwaysHidden();				
					}
					else
					{
						if (GetDistance(pMainInstance) > 0.5f  && !pMainInstance->IsGameMaster())
						{
							m_GraphicThingInstance.ApplyAlwaysHidden();
						}
					}			
	...
	float fScale = ScaleSizeFunc(GetRace());
	if (fScale)
	{
		fScale /= 100.0f;
...
#if defined(__BL_EVENT_STONE_SHAPE_CHANGE__)
DWORD CInstanceBase::GetRealRace() const
{
	return m_dwRealRace;
}
...
void CInstanceBase::SetColorOperation(DWORD dwRace, DWORD dwColor)
{
	if (IsStone())
	{
		if ((dwRace >= 0x1F41 && dwRace <= 0x1F68) || (dwRace >= 0x1F73 && dwRace <= 0x1FB3))
		{
			...
			if (CRaceManager::Instance().GetYutnoriStoneShapeState())
				return;
		}
	}
...
{
	if (mount_vnum == 20251 || mount_vnum == 20246 || mount_vnum == 20247 || mount_vnum == 20252 || mount_vnum == 20254)
	{
		switch (race)
		{
...
#ifdef __LOBBY_MAP_VISIBILITY__
BOOL CInstanceBase::IsInLobbyMap()
{
	if (Frustum::Instance().currentMapNameSave == "metin2_map_lobby")
		return TRUE;
	return FALSE;
...
void CInstanceBase::LoadModel(INT32 model_vnum_start, INT32 model_vnum_end)
{
	for (int i = model_vnum_start; i <= model_vnum_end; i++)
	{
		SelectModel(i);
	}
}

void CInstanceBase::LoadSingleModel(INT32 model_vnum)
{
	SelectModel(model_vnum);
}

void CInstanceBase::SelectModel(const INT32 model_vnum)
{
