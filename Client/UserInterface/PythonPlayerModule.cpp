...
PyObject* playerQuestSkinActive(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildNone();
}
...
PyObject* playerIsInLobbyMap(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pMainInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	if (!pMainInstance)
		return Py_BuildValue("i", FALSE);

	return Py_BuildValue("i", pMainInstance->IsInLobbyMap());
}
#endif
...
PyObject * playerGetStatus(PyObject* poSelf, PyObject* poArgs)
{
	...
	long long iValue = CPythonPlayer::Instance().GetStatus(iType);
...
{
	int64_t i64Gold = CPythonPlayer::Instance().GetStatus(POINT_GOLD);
	return PyLong_FromLongLong(i64Gold);
}

#if defined(ENABLE_GEM_SYSTEM)
PyObject* playerGetGem(PyObject* poSelf, PyObject* poArgs)
{
	int64_t i64gem = CPythonPlayer::Instance().GetStatus(POINT_GEM);
	return PyLong_FromLongLong(i64gem);
}
#endif

PyObject* playerGetInventoryState(PyObject* poSelf, PyObject* poArgs)
{
	int type;
	...
	int amount;
	if (type == 0)
		int amount = CPythonPlayer::Instance().GetStatus(POINT_INVENTORY);
	else if (type == 1)
		int amount = CPythonPlayer::Instance().GetStatus(POINT_SKILL_INVENTORY);
	else if (type == 2)
		int amount = CPythonPlayer::Instance().GetStatus(POINT_MATERIAL_INVENTORY);
	else if (type == 3)
		int amount = CPythonPlayer::Instance().GetStatus(POINT_STONE_INVENTORY);
	else if (type == 4)
		int amount = CPythonPlayer::Instance().GetStatus(POINT_CHEST_INVENTORY);
	else if (type == 5)
		int amount = CPythonPlayer::Instance().GetStatus(POINT_ENCHANT_INVENTORY);
	else
		int amount = CPythonPlayer::Instance().GetStatus(POINT_COSTUME_INVENTORY);
...
{
	int Level = CPythonPlayer::Instance().GetLevel();
...
PyObject * playerGetItemLink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pPlayerItem && CItemManager::Instance().GetItemDataPointer(pPlayerItem->vnum, &pItemData))
	{
		...
		len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%d:%d:%d:%d:%d:%d:%d:%d",
				...
				, pPlayerItem->alSockets[6]
				, pPlayerItem->alSockets[7]
		);
...
PyObject* playerIsSwitchInventorySlot(PyObject* poSelf, PyObject* poArgs)
{
	int iSlotIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotIndex))
		return Py_BuildException();

	char Flag = CPythonPlayer::Instance().IsSwitchInventorySlot(TItemPos(INVENTORY, iSlotIndex));

	return Py_BuildValue("i", Flag);
}

PyObject* playerIsCostumeInventorySlot(PyObject* poSelf, PyObject* poArgs)
{
	int iSlotIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSlotIndex))
		return Py_BuildException();

	char Flag = CPythonPlayer::Instance().IsCostumeInventorySlot(TItemPos(INVENTORY, iSlotIndex));
...
PyObject * playerCanRefine(PyObject * poSelf, PyObject * poArgs)
{
	...
	const bool isFirstCrystalRange = (iTargetItemIndex >= 51010 && iTargetItemIndex <= 51029) ? true : false;
	const bool isSecondCrystalRange = (iTargetItemIndex >= 51030 && iTargetItemIndex <= 51035) ? true : false;
	if (
		(pScrollItemData->GetValue(0) == 7 && !isFirstCrystalRange) || (pScrollItemData->GetValue(0) != 7 && isFirstCrystalRange) ||
		(pScrollItemData->GetValue(0) == 8 && !isSecondCrystalRange) || (pScrollItemData->GetValue(0) != 8 && isSecondCrystalRange)
		)
		return Py_BuildValue("i", REFINE_CANT);
#endif

	const bool bZodiac = ((iTargetItemIndex >= 300 && iTargetItemIndex <= 319) ||
		(iTargetItemIndex >= 1180 && iTargetItemIndex <= 1189) ||
		(iTargetItemIndex >= 2200 && iTargetItemIndex <= 2209) ||
		(iTargetItemIndex >= 3220 && iTargetItemIndex <= 3229) ||
		(iTargetItemIndex >= 5160 && iTargetItemIndex <= 5169) ||
		(iTargetItemIndex >= 7300 && iTargetItemIndex <= 7309) ||
		(iTargetItemIndex >= 19290 && iTargetItemIndex <= 19299) ||
		(iTargetItemIndex >= 19490 && iTargetItemIndex <= 19499) ||
		(iTargetItemIndex >= 19690 && iTargetItemIndex <= 19699) ||
		(iTargetItemIndex >= 19890 && iTargetItemIndex <= 19899)) ? true : false;

	if ((pScrollItemData->GetIndex() == 25042 && bZodiac) || (pScrollItemData->GetIndex() == 25043 && !bZodiac))
		return Py_BuildValue("i", REFINE_CANT);
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
...
#endif
#ifdef __LOBBY_MAP_VISIBILITY__
		{ "IsLobbyMap",					playerIsInLobbyMap,					METH_VARARGS },
#endif
		...
		{ "IsSwitchInventorySlot", playerIsSwitchInventorySlot, METH_VARARGS },
		{ "IsCostumeInventorySlot", playerIsCostumeInventorySlot, METH_VARARGS },
#endif
		///////////////////////////////////////////////////////////////////////////////////////////
...
#endif
		{ "GetInventoryState",			playerGetInventoryState,			METH_VARARGS },
		...
		{ "SetQuestSkinActive",			playerQuestSkinActive,				METH_VARARGS },
		{ NULL,							NULL,								NULL },
	};
	...
	PyModule_AddIntConstant(poModule, "WHEEL_OF_FORTUNE_EVENT", WHEEL_OF_FORTUNE_EVENT);
	PyModule_AddIntConstant(poModule, "HALF_TIMER_DUNGEONS_EVENT", HALF_TIMER_DUNGEONS_EVENT);
	PyModule_AddIntConstant(poModule, "OKEY_EVENT", OKEY_EVENT);
	PyModule_AddIntConstant(poModule, "GOLD_MANNY_EVENT", GOLD_MANNY_EVENT);
#endif
#if defined(ENABLE_GEM_SYSTEM)
	PyModule_AddIntConstant(poModule, "GEM",			POINT_GEM);
#endif
#ifdef ENABLE_BATTLE_PASS
	PyModule_AddIntConstant(poModule, "MISSION_NONE", MISSION_NONE);
	PyModule_AddIntConstant(poModule, "MISSION_BOSS", MISSION_BOSS);
	PyModule_AddIntConstant(poModule, "MISSION_CATCH_FISH", MISSION_CATCH_FISH);
	PyModule_AddIntConstant(poModule, "MISSION_CRAFT_ITEM", MISSION_CRAFT_ITEM);
	PyModule_AddIntConstant(poModule, "MISSION_CRAFT_GAYA", MISSION_CRAFT_GAYA);
	PyModule_AddIntConstant(poModule, "MISSION_DESTROY_ITEM", MISSION_DESTROY_ITEM);
	PyModule_AddIntConstant(poModule, "MISSION_DUNGEON", MISSION_DUNGEON);
	PyModule_AddIntConstant(poModule, "MISSION_EARN_MONEY", MISSION_EARN_MONEY);
	PyModule_AddIntConstant(poModule, "MISSION_FEED_PET", MISSION_FEED_PET);
	PyModule_AddIntConstant(poModule, "MISSION_LEVEL_UP", MISSION_LEVEL_UP);
	PyModule_AddIntConstant(poModule, "MISSION_MOUNT_TIME", MISSION_MOUNT_TIME);
	PyModule_AddIntConstant(poModule, "MISSION_OPEN_OFFLINESHOP", MISSION_OPEN_OFFLINESHOP);
	PyModule_AddIntConstant(poModule, "MISSION_PLAYTIME", MISSION_PLAYTIME);
	PyModule_AddIntConstant(poModule, "MISSION_REFINE_ITEM", MISSION_REFINE_ITEM);
	PyModule_AddIntConstant(poModule, "MISSION_REFINE_ALCHEMY", MISSION_REFINE_ALCHEMY);
	PyModule_AddIntConstant(poModule, "MISSION_SASH", MISSION_SASH);
	PyModule_AddIntConstant(poModule, "MISSION_SELL_ITEM", MISSION_SELL_ITEM);
	PyModule_AddIntConstant(poModule, "MISSION_SPEND_MONEY", MISSION_SPEND_MONEY);
	PyModule_AddIntConstant(poModule, "MISSION_SPRITE_STONE", MISSION_SPRITE_STONE);
	PyModule_AddIntConstant(poModule, "MISSION_STONE", MISSION_STONE);
	PyModule_AddIntConstant(poModule, "MISSION_USE_EMOTICON", MISSION_USE_EMOTICON);
	PyModule_AddIntConstant(poModule, "MISSION_WHISPER", MISSION_WHISPER);
	PyModule_AddIntConstant(poModule, "MISSION_SHOUT_CHAT", MISSION_SHOUT_CHAT);
	PyModule_AddIntConstant(poModule, "MISSION_KILLPLAYER", MISSION_KILLPLAYER);
	PyModule_AddIntConstant(poModule, "MISSION_MONSTER", MISSION_MONSTER);
#endif
	...
	PyModule_AddIntConstant(poModule, "SLOT_TYPE_SWITCH_INVENTORY", SLOT_TYPE_SWITCH_INVENTORY);
	...
	PyModule_AddIntConstant(poModule, "GIFT_BOX_INVENTORY",					GIFT_BOX_INVENTORY );
	PyModule_AddIntConstant(poModule, "SWITCH_INVENTORY",					SWITCH_INVENTORY );
	...
	PyModule_AddIntConstant(poModule, "EFFECT_RAIN",				CPythonPlayer::EFFECT_RAIN);
	PyModule_AddIntConstant(poModule, "EFFECT_FOG",					CPythonPlayer::EFFECT_FOG);
	...
	PyModule_AddIntConstant(poModule, "REWARD_EKZEKIEL", REWARD_EKZEKIEL);
	PyModule_AddIntConstant(poModule, "REWARD_AVERAGE", REWARD_AVERAGE);
	PyModule_AddIntConstant(poModule, "REWARD_INVEN_SLOT", REWARD_INVEN_SLOT);
	PyModule_AddIntConstant(poModule, "REWARD_OFFLINE_SLOT", REWARD_OFFLINE_SLOT);
	PyModule_AddIntConstant(poModule, "REWARD_BATTLEPASS", REWARD_BATTLEPASS);
	PyModule_AddIntConstant(poModule, "REWARD_ACCE", REWARD_ACCE);
	PyModule_AddIntConstant(poModule, "REWARD_LEADERSHIP", REWARD_LEADERSHIP);
	PyModule_AddIntConstant(poModule, "REWARD_CRYSTAL", REWARD_CRYSTAL);
	PyModule_AddIntConstant(poModule, "REWARD_BUFFI", REWARD_BUFFI);
	PyModule_AddIntConstant(poModule, "REWARD_BIOLOG", REWARD_BIOLOG);
	PyModule_AddIntConstant(poModule, "REWARD_ELEMENT_PAGE", REWARD_ELEMENT_PAGE);
	PyModule_AddIntConstant(poModule, "REWARD_FIRST_WEAPON_ZODIAC_CRAFT", REWARD_FIRST_WEAPON_ZODIAC_CRAFT);
	PyModule_AddIntConstant(poModule, "FIRST_AVERAGE_ZODIAC_50", FIRST_AVERAGE_ZODIAC_50);
