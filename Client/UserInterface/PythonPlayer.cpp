...
void CPythonPlayer::__Update_AutoAttack()
{
	...
	// Fix tarnacop (VNUM TARNACOP)
	if (pkInstMain->GetPart(CRaceData::PART_WEAPON) <= 29110 && pkInstMain->GetPart(CRaceData::PART_WEAPON) >= 29101)
		return;

	// Fix undita (VNUM UNDITE)
	if (pkInstMain->GetPart(CRaceData::PART_WEAPON) <= 27590 && pkInstMain->GetPart(CRaceData::PART_WEAPON) >= 27400)
		return;

	// Fix arc fara sageti
	if (pkInstMain->IsBowMode() && !__HasEnoughArrow())
		return;
...
void CPythonPlayer::SetQuickPage(DWORD nQuickPageIndex)
{
...
void CPythonPlayer::RequestMoveGlobalQuickSlotToLocalQuickSlot(DWORD dwGlobalSrcSlotIndex, DWORD dwLocalDstSlotIndex)
{
	...
	rkNetStream.SendQuickSlotMovePacket(dwGlobalSrcSlotIndex, dwGlobalDstSlotIndex);
}
...
void CPythonPlayer::RequestAddLocalQuickSlot(DWORD dwLocalSlotIndex, DWORD dwWndType, DWORD dwWndItemPos)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendQuickSlotAddPacket(dwGlobalSlotIndex, dwWndType, dwWndItemPos);
}
...
void CPythonPlayer::RequestAddToEmptyLocalQuickSlot(DWORD dwWndType, DWORD dwWndItemPos)
{
    for (int i = 0; i < QUICKSLOT_MAX_COUNT_PER_LINE; ++i)
    {
        ...
        if (0 == rkQuickSlot.Type)
        {
            ...
            CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
            rkNetStream.SendQuickSlotAddPacket(dwGlobalQuickSlotIndex, dwWndType, dwWndItemPos);
            return;
        }
    }
}
...
void CPythonPlayer::RequestDeleteGlobalQuickSlot(DWORD dwGlobalSlotIndex)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendQuickSlotDelPacket(dwGlobalSlotIndex);
}
...
void CPythonPlayer::AddQuickSlot(DWORD QuickSlotIndex, DWORD IconType, DWORD IconPosition)
{
...
void CPythonPlayer::DeleteQuickSlot(DWORD QuickSlotIndex)
{
	if (QuickSlotIndex < 0 || QuickSlotIndex >= QUICKSLOT_MAX_NUM)
		return;
...
void CPythonPlayer::MoveQuickSlot(DWORD Source, DWORD Target)
{
...
bool CPythonPlayer::IsSwitchInventorySlot(TItemPos Cell)
{
	return Cell.IsSwitchInventoryCell();
}

bool CPythonPlayer::IsCostumeInventorySlot(TItemPos Cell)
{
	return Cell.IsCostumeInventoryCell();
}
