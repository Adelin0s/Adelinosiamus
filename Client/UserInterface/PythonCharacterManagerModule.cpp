...
PyObject* chrmgrHasAffectByVID(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
	auto pkInstBase = rkChrMgr.GetInstancePtr(iVID);
	if (!pkInstBase)
		return Py_BuildValue("i", 0);

	bool isPoisoned;
	isPoisoned = pkInstBase->HasAffect(iAffectNum) ? true : false;
	return Py_BuildValue("i", isPoisoned);
}
...
PyObject* chrmgrIsPoisoned(PyObject* poSelf, PyObject* poArgs)
{
	DWORD dwVID;
	...
	CPythonCharacterManager& rkChrMgr = CPythonCharacterManager::Instance();
	const CInstanceBase* pkInstBase = rkChrMgr.GetInstancePtr(dwVID);
	if (!pkInstBase)
		return Py_BuildValue("b", false);

	return Py_BuildValue("b", pkInstBase->IsPoisoned());
}
...
void initchrmgr()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "IsPoisoned",					chrmgrIsPoisoned,						METH_VARARGS },
		{ NULL,							NULL,									NULL },
	};
