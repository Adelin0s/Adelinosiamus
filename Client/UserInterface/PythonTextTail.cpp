...
void CPythonTextTail::UpdateTextTail(TTextTail* pTextTail)
{
	...
	if (pTextTail->fDistanceFromPlayer < 300.0f)
	{
		pTextTail->z = 0.0f;
	}
	else
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		if (pTitle)
		{
			...
			if (pLevel)
			{
				...
				if (pLanguageInstance)
				{
					int iLevelWidth, iLevelHeight;
					pLevel->GetTextSize(&iLevelWidth, &iLevelHeight);
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - iTitleWidth - iLevelWidth - pLanguageInstance->GetWidth() - 12.0f + 9.0f, pTextTail->y - 11.0f);
				}
		...
		else
		{
			...
			if (pLevel)
			{
				...
				if (pLanguageInstance)
				{
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - iLevelWidth - pLanguageInstance->GetWidth() - 12.0f + 9.0f, pTextTail->y - 11.0f);
					//pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - pLanguageInstance->GetWidth() - 8.0f, pTextTail->y - 10.0f);
				}
...
void CPythonTextTail::ShowAllTextTail()
{
	...
	if (GetAsyncKeyState(VK_MENU) & 0x8000)
	{
		for (itor = m_CharacterTextTailMap.begin(); itor != m_CharacterTextTailMap.end(); ++itor)
		{
			...
			if (pTextTail->fDistanceFromPlayer < 7000.0f)
	...
	else
	{
		...
		{
			TTextTail* pTextTail = itor->second;
			if (pTextTail->fDistanceFromPlayer < CPythonSystem::instance().GetTEXT())
				ShowCharacterTextTail(itor->first);
		}
		for (itor = m_ItemTextTailMap.begin(); itor != m_ItemTextTailMap.end(); ++itor)
		{
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
	...
	if (!prLanguage && CPythonSystem::Instance().GetPlayerFlag())
	{
