...
PyObject* renderGetFreeIndex(PyObject* poSelf, PyObject* poArgs)
{
	...
	int lastValue = iMin + 20;

	return Py_BuildValue("i", lastValue);
}
...
PyObject* renderSetMotionStatus(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BadArgument();
	int iStatus;
	if (!PyTuple_GetInteger(poArgs, 1, &iStatus))
		return Py_BadArgument();
	const std::shared_ptr<CRenderTarget> target = CRenderTargetManager::Instance().GetRenderTarget(iIndex);
	if (target)
		target->SetMotionStatus(iStatus ? true : false);
	return Py_BuildNone();
}

PyObject* renderSetEffectStatus(PyObject* poSelf, PyObject* poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BadArgument();
	...
	const std::shared_ptr<CRenderTarget> target = CRenderTargetManager::Instance().GetRenderTarget(iIndex);
	if (target)
		target->SetEffectStatus(iStatus ? true : false);
	return Py_BuildNone();
}
...
#endif
		{"SetMotionStatus", renderSetMotionStatus, METH_VARARGS },
		{"SetEffectStatus", renderSetEffectStatus, METH_VARARGS },
