...
#else
		m_pyGraphic.SetPerspective(45.0f, fAspect, 100.0, fFarClip);
#endif
...
#else
		s.SetPerspective(45.0f, fAspect, 100.0f, fFarClip);
#endif
		s.BuildViewFrustum();
	}
...
bool CPythonApplication::Create(PyObject* poSelf, const char* c_szName, int width, int height, int Windowed)
{
	...
	TraceError("%u %u", m_pySystem.GetBPP(), m_pySystem.GetFrequency());
	// Device
