...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_SET_MULTIPLE_ITEM:
				ret = RecvSetMultipleItem();
				break;
...
{
	Frustum::Instance().instance().isWarp = true;
	Frustum::Instance().instance().chunkLoadCounter = 0;
...
#if defined(__BL_67_ATTR__)
bool CPythonNetworkStream::Send67AttrPacket(DWORD iMaterialCount, DWORD iSupportCount, int iSupportPos, int iItemPos)
{
	TPacketCG67Attr p =
	{
		static_cast<BYTE>(HEADER_CG_67_ATTR),
		static_cast<DWORD>(iMaterialCount),
		static_cast<DWORD>(iSupportCount),
...
bool CPythonNetworkStream::SendItemCombinationPacket(const int iMediumIndex, const int iBaseIndex, const int iMaterialIndex)
{
	...
	p.MediumIndex = (short)iMediumIndex;
	p.BaseIndex = (short)iBaseIndex;
	p.MaterialIndex = (short)iMaterialIndex;
		...
		case SHOP_SUBHEADER_GC_START_EX:
			{
				...
				for (unsigned char i = 0; i < shop_tab_count; i++)
				{
					...
					{
						CPythonShop::Instance().SetItemData(i, j, pPackTab->items[j]);
					}
				}
...
bool CPythonNetworkStream::RecvRefineInformationPacketNew()
{
	...
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME],
		...
		Py_BuildValue("(iiiiii)",
			...
			kRefineInfoPacket.refine_table.refinement_type)
		);
...
void CPythonNetworkStream::Discord_Update(const bool ingame)
{
	...
	discordPresence.buttonLabel = "Go to the Website";
	discordPresence.buttonURL = "https://zenaris.io";
#endif
...
bool CPythonNetworkStream::RecvShopSearchSet()
{
	...
	int pageIdx;
	if (!Recv(sizeof(int), &pageIdx))
		return false;
	int totalPageCount;
	if (!Recv(sizeof(int), &totalPageCount))
		return false;
	int itemCount;
	if (!Recv(sizeof(int), &itemCount))
		return false;
	...
	if (itemCount)
	{
		...
		if (!Recv(itemCount * sizeof(TOfflineShopItemData), m_vec.data()))
			return false;
		for (WORD j = 0; j < m_vec.size(); j++)
		{
			if (m_vec[j].vnum != 0)
				CPythonPrivateShopSearch::Instance().AddItemData(m_vec[j]);
		}
	}
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RefreshShopSearch", Py_BuildValue("(ii)", pageIdx, totalPageCount));
	return true;
}
#endif
...
bool CPythonNetworkStream::RecvOfflineShopPacket()
{
	...
	case SHOP_SUBHEADER_GC_ITEM_OPTIMIZE:
	case SHOP_SUBHEADER_GC_LOG_OPTIMIZE:
		break;
	default:
		if (iSize > 0)
		{
	...
	switch (packet_shop.subheader)
	{
	case SHOP_SUBHEADER_GC_CLEAR_DATA:
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "ClearOfflineShopLog", Py_BuildValue("()"));
		break;

	case SHOP_SUBHEADER_GC_ITEM_OPTIMIZE:
	{
		std::vector<TOfflineShopItemData> tmpVec;
		tmpVec.resize(packet_shop.packet_size);
		if (!Recv(sizeof(TOfflineShopItemData) * packet_shop.packet_size, tmpVec.data()))
			return false;
		for (BYTE iItemIndex = 0; iItemIndex < packet_shop.packet_size; ++iItemIndex)
			CPythonShop::Instance().SetOfflineShopItemData(tmpVec[iItemIndex].display_pos, tmpVec[iItemIndex]);
	}
	break;

	case SHOP_SUBHEADER_GC_LOG_OPTIMIZE:
	{
		std::vector<ShopLog> tmpVec;
		tmpVec.resize(packet_shop.packet_size);
		if (!Recv(sizeof(ShopLog) * packet_shop.packet_size, tmpVec.data()))
			return false;
		for (BYTE iItemIndex = 0; iItemIndex < packet_shop.packet_size; ++iItemIndex)
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AppendShopLog", Py_BuildValue("(ssiiLi)", tmpVec[iItemIndex].name, tmpVec[iItemIndex].date, tmpVec[iItemIndex].itemVnum, tmpVec[iItemIndex].itemCount, tmpVec[iItemIndex].price, 0));
	}
	break;
	...
	case SHOP_SUBHEADER_GC_UPDATE_ITEM:
	{
		...
		if (pShopUpdateItemPacket->log.price != 0) {
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AppendShopLog", Py_BuildValue("(ssiiLi)", pShopUpdateItemPacket->log.name, pShopUpdateItemPacket->log.date, pShopUpdateItemPacket->log.itemVnum, pShopUpdateItemPacket->log.itemCount, pShopUpdateItemPacket->log.price, 1));
		}
