...
void CInstanceBase::AddDamageEffect(INT damage,BYTE flag,BOOL bSelf,BOOL bTarget)
{
...
bool CInstanceBase::ProcessDamage()
{
	...
	{
		std::vector<string> currentTexture = { textures[i] };
		rkEftMgr.SetEffectTextures(ms_adwCRCAffectEffect[rdwCRCEft], currentTexture);
		...
		D3DXMatrixTranslation(&matTrans, FONT_WIDTH * i, 0, 0);
		...
		DWORD dwEID = rkEftMgr.CreateEffect(ms_adwCRCAffectEffect[rdwCRCEft], D3DXVECTOR3(matrix._41, matrix._42, matrix._43)
			, v3Rot);

		m_createdEffects[dwEID] = std::chrono::steady_clock::now();
	}

	return true;
}

void CInstanceBase::RemoveExpiredEffects(std::chrono::milliseconds duration)
{
	CEffectManager& rkEftMgr = CEffectManager::Instance();
	...
	{
		if (std::chrono::duration_cast<std::chrono::milliseconds>(currentTime - it->second) >= duration)
		{
			__DetachEffect(it->first);
...
#ifdef ENABLE_NEW_PET_SYSTEM
	else if (IsPet() || IsNewPet() || IsHorse())
	{
		return NAMECOLOR_NPC;
	}
#endif
...
void CInstanceBase::AttachTextTail()
{
	...
	// CHARACTER_LEVEL ( DEZACTIVARE SISTEM CAL )
...
{
	if (IsStone() && GetVirtualNumber() != 6118)
	{
...
void CInstanceBase::SetAffectFlagContainer(const CAffectFlagContainer& c_rkAffectFlagContainer)
{
	...
	else if (IsStone() && GetVirtualNumber() != 6118)
	{
	...
	else
	{
		...
		if (!__MainCanSeeHiddenThing() && (c_rkAffectFlagContainer.IsSet(AFFECT_INVISIBILITY) || c_rkAffectFlagContainer.IsSet(AFFECT_REVIVE_INVISIBILITY)))
