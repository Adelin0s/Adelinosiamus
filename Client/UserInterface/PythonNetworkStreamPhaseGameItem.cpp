...
bool CPythonNetworkStream::RecvSetMultipleItem()
{
	TPacketGCSetMultipleItem packet;

	if (!Recv(sizeof(TPacketGCSetMultipleItem), &packet))
		return false;

	for (int i = 0; i < packet.items.size(); i++)
	{
		TItemData kItemData;
		kItemData.vnum = packet.items[i].vnum;
		kItemData.count = packet.items[i].count;
		kItemData.flags = packet.items[i].flags;
		kItemData.anti_flags = packet.items[i].anti_flags;

		for (int p = 0; p < ITEM_SOCKET_SLOT_MAX_NUM; ++p)
			kItemData.alSockets[p] = packet.items[p].alSockets[p];

		for (int j = 0; j < ITEM_ATTRIBUTE_SLOT_MAX_NUM; ++j)
			kItemData.aAttr[j] = packet.items[j].aAttr[j];


		IAbstractPlayer& rkPlayer = IAbstractPlayer::GetSingleton();
		rkPlayer.SetItemData(packet.items[i].Cell, kItemData);
	}
...
bool CPythonNetworkStream::RecvQuickSlotAddPacket()
{
	...
	DWORD pos = packet_quick_slot_add.pos;
	DWORD type = packet_quick_slot_add.slot.Type;
	DWORD position = packet_quick_slot_add.slot.Position;

	IAbstractPlayer& rkPlayer=IAbstractPlayer::GetSingleton();
	rkPlayer.AddQuickSlot(pos, type, position);
...
bool CPythonNetworkStream::SendShopSellPacketNew(WORD bySlot, DWORD byCount)
{
	...
	Tracef(" SendShopSellPacketNew(bySlot=%d)\n", bySlot);
...
bool CPythonNetworkStream::SendQuickSlotAddPacket(DWORD wpos, DWORD type, DWORD pos)
{
	if (!__CanActMainInstance())
		return true;
	...
	quickSlotAddPacket.slot.Position = (DWORD)pos;
...
bool CPythonNetworkStream::SendQuickSlotDelPacket(DWORD pos)
{
	if (!__CanActMainInstance())
		return true;
...
bool CPythonNetworkStream::SendQuickSlotMovePacket(DWORD pos, DWORD change_pos)
{
	if (!__CanActMainInstance())
		return true;
