...
PyObject* itemGetItemSummary2(PyObject* poSelf, PyObject* poArgs)
{
	CItemData* pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	if (!pItemData)
		return Py_BuildException("no selected item data");

	return Py_BuildValue("s", pItemData->GetSummary2());
}

PyObject* itemGetItemSummary3(PyObject* poSelf, PyObject* poArgs)
{
	CItemData* pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	if (!pItemData)
		return Py_BuildException("no selected item data");

	return Py_BuildValue("s", pItemData->GetSummary3());
}

PyObject* itemGetItemSummary4(PyObject* poSelf, PyObject* poArgs)
{
	CItemData* pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	if (!pItemData)
		return Py_BuildException("no selected item data");

	return Py_BuildValue("s", pItemData->GetSummary4());
}
...
PyObject * itemIsRefineScroll(PyObject * poSelf, PyObject * poArgs)
{
	...
	int iDestItemIndex;
	if (!PyTuple_GetInteger(poArgs, 1, &iDestItemIndex))
		return Py_BadArgument();

	CItemManager::Instance().SelectItemData(iDestItemIndex);
	CItemData * pItemData = CItemManager::Instance().GetSelectedItemDataPointer();
	...
	pItemData->GetLimit(0, &limit);

	bool bZodiac;

	if ((iDestItemIndex >= 300 && iDestItemIndex <= 319) || 
			(iDestItemIndex >= 1180 && iDestItemIndex <= 1189) || 
			(iDestItemIndex >= 2200 && iDestItemIndex <= 2209) || 
			(iDestItemIndex >= 3220 && iDestItemIndex <= 3229) || 
			(iDestItemIndex >= 5160 && iDestItemIndex <= 5169) || 
			(iDestItemIndex >= 7300 && iDestItemIndex <= 7309) || 
			(iDestItemIndex >= 19290 && iDestItemIndex <= 19299) || 
			(iDestItemIndex >= 19490 && iDestItemIndex <= 19499) || 
			(iDestItemIndex >= 19690 && iDestItemIndex <= 19699) || 
			(iDestItemIndex >= 19890 && iDestItemIndex <= 19899))
	{
		bZodiac = true;
	}
	else { 
	...
	if (71021 == iItemIndex && limit.lValue <= 75 && bZodiac == false)
		return Py_BuildValue("i", TRUE);
	if (351220 == iItemIndex && limit.lValue <= 75 && bZodiac == false)
		return Py_BuildValue("i", TRUE);
	if (25041 == iItemIndex)
		return Py_BuildValue("i", TRUE);
	if (25042 == iItemIndex && bZodiac == false)
		return Py_BuildValue("i", TRUE);
	if (25042 == iItemIndex && bZodiac == true)
		return Py_BuildValue("i", FALSE);
	if (25043 == iItemIndex && bZodiac == true)
		return Py_BuildValue("i", TRUE);
	if (25043 == iItemIndex && bZodiac == false)
		return Py_BuildValue("i", FALSE);
	if (70039 == iItemIndex)
		return Py_BuildValue("i", TRUE);
	if (71032 == iItemIndex && bZodiac == false)
		return Py_BuildValue("i", TRUE);
	if (25040 == iItemIndex)
		return Py_BuildValue("i", TRUE);
...
PyObject * itemCanAddToQuickSlotItem(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pItemData->GetIndex() == 70058 || pItemData->GetIndex() == 70038)
		return Py_BuildValue("i", TRUE);
...
PyObject* itemGetIconImageByVnum(PyObject* poSelf, PyObject* poArgs)
{
...
void initItem()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetIconImageByVnum",				itemGetIconImageByVnum,					METH_VARARGS },
		...
		{ "GetItemSummary2",				itemGetItemSummary2,					METH_VARARGS },
		{ "GetItemSummary3",				itemGetItemSummary3,					METH_VARARGS },
		{ "GetItemSummary4",				itemGetItemSummary4,					METH_VARARGS },
	...
	PyModule_AddIntConstant(poModule, "COSTUME_SLOT_PET", c_Slot_Pet);
	PyModule_AddIntConstant(poModule, "COSTUME_SLOT_NORMAL_PET", c_Slot_Normal_Pet);
#endif

	PyModule_AddIntConstant(poModule, "COSTUME_ACCE_SKIN", c_Slot_Acce_Skin);
	PyModule_AddIntConstant(poModule, "COSTUME_MOUNT_SKIN", c_Slot_Mount_Skin);
	PyModule_AddIntConstant(poModule, "COSTUME_NORMAL_PET_SKIN", c_Slot_Normal_Pet_Skin);

#if defined(ENABLE_AURA_COSTUME_SYSTEM)
	PyModule_AddIntConstant(poModule, "COSTUME_SLOT_AURA", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "INVENTORY_MAX_NUM", c_ItemSlot_Count);

#ifdef __PENDANT__
	PyModule_AddIntConstant(poModule, "PENDANT_FIRE_SLOT", c_EquipmentPendantFire);
	PyModule_AddIntConstant(poModule, "PENDANT_ICE_SLOT", c_EquipmentPendantIce);
	PyModule_AddIntConstant(poModule, "PENDANT_THUNDER_SLOT", c_EquipmentPendantThunder);
	PyModule_AddIntConstant(poModule, "PENDANT_WIND_SLOT", c_EquipmentPendantWind);
	PyModule_AddIntConstant(poModule, "PENDANT_DARK_SLOT", c_EquipmentPendantDark);
	PyModule_AddIntConstant(poModule, "PENDANT_EARTH_SLOT", c_EquipmentPendantEarth);
	PyModule_AddIntConstant(poModule, "PENDANT_FIRST_SOUL_SLOT", c_EquipmentPendantFirst);
	PyModule_AddIntConstant(poModule, "PENDANT_SECOND_SOUL_SLOT", c_EquipmentPendantSecond);
	...
	PyModule_AddIntConstant(poModule, "ARMOR_PENDANT_SOUL", CItemData::ARMOR_PENDANT_SOUL);
#endif
	...
	PyModule_AddIntConstant(poModule, "SWITCH_INVENTORY_SLOT_START", c_Switch_Inventory_Slot_Start);
	PyModule_AddIntConstant(poModule, "COSTUME_INVENTORY_SLOT_START", c_Costume_Inventory_Slot_Start);
#endif
	...
	PyModule_AddIntConstant(poModule, "ITEM_FLAG_STACKEBLE", CItemData::ITEM_FLAG_TIME_STACKEBLE);
#endif
#ifdef __TIME_STACKEBLE__
	PyModule_AddIntConstant(poModule, "ITEM_FLAG_TIME_STACKEBLE", CItemData::ITEM_FLAG_TIME_STACKEBLE);
#endif
	...
	PyModule_AddIntConstant(poModule, "APPLY_CRITICAL_DAMAGE_PVM", CItemData::APPLY_CRITICAL_DAMAGE_PVM);
	PyModule_AddIntConstant(poModule, "APPLY_RESIST_RACES", CItemData::APPLY_RESIST_RACES);
	...
	PyModule_AddIntConstant(poModule, "APPLY_PARTY_ATTACKER_BONUS", CItemData::APPLY_PARTY_ATTACKER_BONUS);
	PyModule_AddIntConstant(poModule, "APPLY_PARTY_HASTE_BONUS", CItemData::APPLY_PARTY_HASTE_BONUS);
	PyModule_AddIntConstant(poModule, "APPLY_PARTY_TANKER_BONUS", CItemData::APPLY_PARTY_TANKER_BONUS);
	PyModule_AddIntConstant(poModule, "APPLY_PARTY_DEFENDER_BONUS", CItemData::APPLY_PARTY_DEFENDER_BONUS);
	PyModule_AddIntConstant(poModule, "APPLY_PARTY_BUFFER_BONUS", CItemData::APPLY_PARTY_BUFFER_BONUS);
	PyModule_AddIntConstant(poModule, "APPLY_PARTY_SKILL_MASTER_BONUS", CItemData::APPLY_PARTY_SKILL_MASTER_BONUS);
	PyModule_AddIntConstant(poModule, "APPLY_RESIST_NORMAL_DAMAGE", CItemData::APPLY_RESIST_NORMAL_DAMAGE);
