...
void CPythonChat::AppendChatLog(int iType, const char* c_szChat)
{
	...
	m_ChatLogLineDeque.push_back(pChatLine);
	...
	for (TChatSetMap::iterator itor = m_ChatSetMap.begin(); itor != m_ChatSetMap.end(); ++itor)
	{
		...
		if (!pChatSet->CheckMode(pChatLine->iType))
			continue;
#endif
		pChatSet->m_ShowingChatLogLineList.push_back(pChatLine);
...
void CPythonChat::ArrangeShowingChatLog(DWORD dwID)
{
	...
	for (int i = min(icurLineCount - 1, iEndLine); i >= 0; --i)
	{
		...
		pChatSet->m_ShowingChatLogLineList.push_front(TempChatLineDeque[i]);
		iHeight += pChatSet->m_iStep;
	}
}
#endif
...
void CWhisper::Save(const char* c_szMain, const char* c_szName)
{
	...
	snprintf(chatLogFileName, sizeof(chatLogFileName), "data/user_data/pm_temp/%s/%s", c_szMain, c_szName);

	FILE* file = fopen(chatLogFileName, "w");
	if (!file)
		return;
	fclose(file);
