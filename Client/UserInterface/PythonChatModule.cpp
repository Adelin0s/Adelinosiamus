...
PyObject* chatGetLinkFromHyperlink(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (0 == results[0].compare("item"))
	{
		...
		if (CItemManager::Instance().GetItemDataPointer(htoi(results[1].c_str()), &pItemData))
		{
			...
			len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%lld:%lld:%lld:%lld:%lld:%lld:%lld:%lld",
				htoi(results[1].c_str()),
				htoi(results[2].c_str()),
				atoi(results[3].c_str()), // socket0
				atoi(results[4].c_str()),
				atoi(results[5].c_str()), // last good
				atoi(results[6].c_str()),
				atoi(results[7].c_str()),
				atoi(results[8].c_str()),
				atoi(results[9].c_str()),
				atoi(results[10].c_str()));
...
void initChat()
{
	...
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_DICE_INFO",	0);
#endif
