...
{
	PrintMe(1, "__CheckMilesFiles processing . and .\\data/miles");
	if (checkMilesDir(".") || checkMilesDir(".\\data/miles"))
		return false;
	return true;
}
#endif
...
const std::string& ApplicationStringTable_GetString(DWORD dwID, LPCSTR szKey)
{
	...
	strcat(szIniFileName, "zenaris.dat");
...
bool Main(HINSTANCE hInstance, LPSTR lpCmdLine)
{
	...
	m_pyConfig.Initialize("config/config.cfg");
#endif
...
#include <cstdlib>
#include <cstring>
#include <ShlObj.h> // Include the necessary header for CSIDL_APPDATA


int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
#ifdef CHECK_AUTOPATCHER
	// Check if the application was launched with the argument "--ghf3Nqww5N8D4qB62s3vk5by"
	if (strstr(lpCmdLine, PATCHER_ARG) != nullptr) {
		// Get the path to the AppData directory
		PWSTR appDataPath = nullptr;
		if (SUCCEEDED(SHGetFolderPathW(nullptr, CSIDL_APPDATA, nullptr, 0, appDataPath))) {
			// Convert the path to ANSI format
			char appDataPathA[MAX_PATH];
			if (WideCharToMultiByte(CP_ACP, 0, appDataPath, -1, appDataPathA, MAX_PATH, nullptr, nullptr)) {
				// Concatenate the file name to the AppData path
				strcat_s(appDataPathA, "\\4hfptvfi33a3ffo");

				// Check if the "4hfptvfi33a3ffo" file exists and get the time from it
				std::ifstream timeFile(appDataPathA);
				...
				if (timeFile.is_open()) {
					...
					// If the file doesn't exist, create it and set the time to zero
					std::ofstream newTimeFile(appDataPathA);
					if (newTimeFile.is_open()) {
						lastUpdateTime = 0;
						newTimeFile << lastUpdateTime;
						newTimeFile.close();
					}
					else {
						// If the file cannot be created, display an error message
						std::cerr << "Unable to create or write to the file " << appDataPathA << std::endl;
					}
				}

				// Calculate the time difference between the last update and the current time
				std::time_t currentTime = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
				std::time_t timeDifference = currentTime - lastUpdateTime;

				// Check if the update time is greater than 12 hours (43200 seconds)
				if (timeDifference >= 43200) {
					// Run the autopatcher
					int result = std::system("Zenaris_Patcher.exe");
					if (result != 0) {
						std::cerr << "Error running Zenaris_Patcher.exe." << std::endl;
					}
			...
			// The application was not launched with the argument
	...
	if (strstr(lpCmdLine, ARGUMENT_STARTUP) == 0) {
		...
		std::string msg = std::string("") + std::string(ARGUMENT_PATCHER_NAME);
		MessageBoxA(NULL, msg.c_str(), "NumeStarter", MB_OK);
	}
#endif
#endif
