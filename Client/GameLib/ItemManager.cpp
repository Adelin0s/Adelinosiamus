...
bool CItemManager::LoadItemDesc(const char* c_szFileName)
{
	...
	for (DWORD i = 0; i < kTextFileLoader.GetLineCount(); ++i)
	{
		...
		const std::string& c_rstSumm2 = kTokenVector[ITEMDESC_COL_SUMM2];
		const std::string& c_rstSumm3 = kTokenVector[ITEMDESC_COL_SUMM3];
		const std::string& c_rstSumm4 = kTokenVector[ITEMDESC_COL_SUMM4];
		...
		pkItemDataFind->SetSummary2(__SnapString(c_rstSumm2, stTemp));
		pkItemDataFind->SetSummary3(__SnapString(c_rstSumm3, stTemp));
		pkItemDataFind->SetSummary4(__SnapString(c_rstSumm4, stTemp));
	}
	return true;
}
...
bool CItemManager::LoadItemTable(const char* c_szFileName)
{
	...
	else if (dwFourCC != MAKEFOURCC('M', 'I', 'P', 'X'))
	{
